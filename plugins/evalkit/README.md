...
`skills/tashtit-evalkit/SKILL.md` is a router that dispatches to six behaviors:

| Skill | Kind | Cost | What it does |
| --- | --- | --- | --- |
| `tashtit-review-skill` | Static | Cheap | Reviews one skill's authoring quality by reading its files. |
| `tashtit-compare-skills` | Static | Cheap | Reviews two skills and synthesizes a difference summary. |
| `tashtit-evaluate-skill` | Dynamic | Expensive | Runs a task with a skill present and with it deleted, then diffs the outcomes. |
| `tashtit-benchmark-skills` | Dynamic | Expensive | Runs a task under each of two skills, one per arm, then diffs the outcomes. |
| `tashtit-compare-models` | Dynamic | Expensive | Runs a task under each of two models, one per arm, with an arm- and model-blind reviewer. |
| `tashtit-remove-worktrees` | Utility | Cheap | Lists and removes worktrees and branches left by the dynamic harnesses. |
...
- Dynamic harnesses confirm before spawning headless sessions when the request is
  ...
  not inferred.
- `tashtit-remove-worktrees` defaults to **list only**; it removes nothing without an
...
- Reviewers and judges are **arm-blind** (and **model-blind** for `tashtit-compare-models`)
  so the label cannot bias the finding.

## Prerequisites

- The bundled `skill-reviewer` agent (`agents/skill-reviewer.md`) backs
  `tashtit-review-skill` and `tashtit-compare-skills`. It ships with this plugin and is
...
| Threat | Control |
| --- | --- |
| Loss of uncommitted or unpushed work during cleanup | `tashtit-remove-worktrees` defaults to listing, excludes the main worktree, and confirms individually before removing any dirty worktree |
