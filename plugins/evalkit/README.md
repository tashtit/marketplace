...
| Invalid control arm | Both dynamic skills pin each arm's skill state — a loose skill by file delete, a plugin skill by writing the host's highest-precedence settings file into the worktree to force `enabledPlugins` on/off — and stop if a plugin's files are missing or a managed scope force-enables it. The pin is excluded from the arm's staged result so it never reaches the arm-blind reviewer or the `files_changed` count. A per-arm **fired-check** confirms the skill actually fired where present and not where absent; a run that never triggered the skill is reported inconclusive, not "no effect" |
