...
When invoked as `/review-skill <skill>`, read it from `$ARGUMENTS`. When triggered from a natural-language request, take the skill named in the user's message. If no skill is identified, ask which one rather than guessing.
...
- **Static only.** This makes no claim about task performance. A well-authored skill can still fail to improve outcomes — use `evaluate-skill` for that question.
