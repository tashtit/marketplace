...
When invoked as `/tashtit-compare-models <task…>`, parse `task` from `$ARGUMENTS` as all positional text. When triggered from a natural-language request, take the task from the user's message. **If no task is given, ask for it — never guess one.**
...
9. Worktrees are **always kept**. Never auto-remove — use `tashtit-remove-worktrees` to clean up later.
