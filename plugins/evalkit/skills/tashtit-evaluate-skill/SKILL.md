...
When invoked as `/tashtit-evaluate-skill <skill> <task…>`, parse from `$ARGUMENTS`: `skill` is positional arg 1 and `task` is the remaining positional text. When triggered from a natural-language request, extract the same values from the user's message.
