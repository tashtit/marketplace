---
name: tashtit-remove-worktrees
description: "Usage: /tashtit-remove-worktrees [list | all | --prefix <p> | <name>…] [--keep-branches] — no args lists only; nothing is removed without an explicit target. List and remove git worktrees and their associated branches, with a confirmation gate for any worktree holding uncommitted or unpushed work. Defaults to listing only. Use when asked to list, clean up, or remove worktrees — including leftovers from tashtit-evaluate-skill, tashtit-benchmark-skills, or tashtit-compare-models runs."
---
...
When invoked as `/tashtit-remove-worktrees [args]`, parse from `$ARGUMENTS`. When triggered from a natural-language request, infer the mode from the user's message — but **default to `list` whenever the intent is not unambiguously destructive**, and show the user what would be removed before removing it.
